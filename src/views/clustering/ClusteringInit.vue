...
const maxLoop = ref(50)
const alpha = ref(2.0)
const m_range = ref([1.1, 9.1])
...
const onCluster = async () => {
  ...
  const body = {
    config: {
      ...
      max_loop: maxLoop.value,
      alpha: alpha.value,
      upper_m: m_range.value[1],
      lower_m: m_range.value[0]
    },
    filter: {
  ...
  const resp = await axios.post(`${BASE_URL}/api/v1/clustering/start`, body, {
    withCredentials: true
  })
  if (resp.data.error_code === 0) {
...
<template>
  <div style="width: 100%; min-height: 850px; padding: 20px">
    ...
    <v-container fluid style="border: 1px solid black; margin-top: 20px; margin-bottom: 20px">
      ...
      <v-row>
        <v-col cols="6">
          ...
          <draggable tag="ul" :list="orderList" class="list-group" item-key="name">
        ...
        <v-col cols="6">
          ...
          <v-text-field v-model="numCluster" type="number"> </v-text-field>
          ...
          <v-text-field
            ...
            v-bind:rules="[
              (value) =>
                value * numCluster >= totalItem ||
                'Max size must be higher than Current Total / Cluster Number.'
            ]"
          >
          </v-text-field>
        </v-col>
      ...
      <v-row>
        <v-col>
          <v-list>
            <v-list-group value="Advance settings">
              ...
              <v-list-item>
                <span class="text-h6">Max loops</span>
                <v-text-field v-model="maxLoop" type="number" style="width: 400px" />
              </v-list-item>
              <v-list-item>
                <span class="text-h6">Alpha</span>
                <v-text-field v-model="alpha" type="number" style="width: 400px" />
              </v-list-item>
              <v-list-item>
                <span class="text-h6">M Range</span
                ><v-range-slider
                  v-model="m_range"
                  strict
                  :min="1.1"
      ...
      <v-row>
        <v-col cols="4"
          ><span class="text-h5"
            >Filter thesis to cluster (Current total: {{ totalItem }})</span
          ></v-col
        >
      </v-row>
      <v-row>
    ...
    <v-dialog v-model="historyDialog" width="auto" style="font-size: 20px" persistent>
      <v-card class="pa-3">
        <v-card-title class="text-center mb-5"> Begin clustering with this config? </v-card-title>
        <v-card-actions>
          <v-btn color="red-darken-3" @click="historyDialog = false"
            ><strong style="font-size: 20px">Cancel</strong></v-btn
          >
          <v-spacer></v-spacer>
          <v-btn color="green-darken-3" @click="onCluster"
