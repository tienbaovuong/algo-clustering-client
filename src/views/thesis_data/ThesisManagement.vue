...
const onDelete = async () => {
  ...
  await axios.delete(`${BASE_URL}/api/v1/thesis_data/${deleteId.value}`, {
    withCredentials: true
  })
...
<template>
  <div style="width: 100%; min-height: 850px">
    <v-container fluid>
      <v-row>
        ...
        <v-col class="d-flex align-center justify-end" cols="8">
          <v-btn color="blue-darken-3" elevation="2" @click.stop="drawer = !drawer">Upload </v-btn>
        </v-col>
      </v-row>
      <v-row>
        <v-col class="d-flex">
    ...
    <v-navigation-drawer v-model="drawer" location="right" temporary width="800px">
      <v-list>
